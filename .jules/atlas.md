...
## 2026-03-05 - Unwinding API/Core Dependency (TxId)\n**Tangle:** `api` re-exported `TxId` from `core::id`. `storage` and `tests` modules imported `TxId` via `api::transaction::TxId` instead of directly using `core::id::TxId`. This caused an architectural smell (a leaky abstraction) where an `api` type leaked an implementation detail from `core`, creating confusing dependency paths and potential cycles.\n**Blueprint:** Removed `TxId` re-export from `api::transaction::types`, `api::transaction::mod`, and `api::mod`. Added `pub use core::id::TxId` directly to `src/lib.rs` so users still have top-level access. Updated all internal references in `api/transaction` and tests to explicitly import `TxId` directly from `crate::core::id::TxId`.
