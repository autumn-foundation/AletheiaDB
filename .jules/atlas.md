...
## 2026-06-21 - Splitting Temporal Vector Index Tests Blob
**Tangle:** `src/index/vector/temporal/mod.rs` contained both core logic and over 2300 lines of tests. It made navigation and understanding the core operations difficult, creating a "Blob".
**Blueprint:** Moved all tests inside `mod tests { ... }` in `src/index/vector/temporal/mod.rs` to a new sibling module `src/index/vector/temporal/tests.rs` with `#[cfg(test)]`. This maintains test functionality while physically separating testing code from domain logic.
