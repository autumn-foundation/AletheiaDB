...
## 2026-05-26 - Splitting Large Blob Modules
**Tangle:** Several modules (`src/core/version.rs`, `src/storage/checkpoint.rs`, `src/storage/migration.rs`, `src/query/parser.rs`, `src/core/temporal.rs`, `src/index/current.rs`, `src/core/id.rs`) were excessively long "Blobs", mixing core logic with thousands of lines of tests. This violated the Single Responsibility Principle and made navigation and maintenance difficult.
**Blueprint:** Refactored these modules into their respective directories (e.g., `src/core/version/`). Kept the core data definitions and logic in `mod.rs` and extracted all tests to `tests.rs`. This physically separates concerns while maintaining the `#[cfg(test)]` functionality.
