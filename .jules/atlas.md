...
## 2026-06-21 - Splitting Temporal Vector Index Blob
**Tangle:** `src/index/vector/temporal/mod.rs` was over 3700 lines long, a "Blob" module holding both core temporal vector indexing logic and over 2300 lines of tests. It made navigation and understanding the core operations difficult.
**Blueprint:** Extracted tests into `src/index/vector/temporal/tests.rs` (~2300 lines), reducing `mod.rs` to ~1400 lines while maintaining the `#[cfg(test)]` block functionality but with better physical separation.
