...
## 2026-06-22 - Extracting Tests from Blob Modules
**Tangle:** Several modules (`src/index/vector/distributed.rs`, `src/index/vector/sparse.rs`, `src/index/vector/temporal/mod.rs`, `src/query/converter.rs`, `src/query/executor/iterators.rs`, `src/query/parser.rs`, `src/query/planner/physical.rs`, `src/storage/checkpoint.rs`, `src/storage/migration.rs`, `src/storage/wal/segment_reader.rs`) were "Blob" files that contained thousands of lines, largely due to inline `#[cfg(test)] mod tests { ... }` blocks mixed with core logic, making the code extremely hard to navigate and maintain.
**Blueprint:** Refactored each of these files into their own module directory (e.g., `src/query/parser/`). Kept the core data definitions and implementation in `mod.rs` and moved all tests to `tests.rs`, maintaining the `#[cfg(test)]` block functionality but with better physical separation.
