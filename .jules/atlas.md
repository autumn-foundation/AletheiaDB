...
**Blueprint:** Extracted the 2300+ lines of `#[cfg(test)]` into `src/index/vector/temporal/tests.rs`, maintaining the `#[cfg(test)]` block functionality but with better physical separation. Replaced the inline block with `#[cfg(test)]\nmod tests;`.
