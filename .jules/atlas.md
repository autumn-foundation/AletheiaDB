...
## 2026-06-21 - Splitting Executor Iterators Blob
**Tangle:** `src/query/executor/iterators.rs` was over 3400 lines long, a "Blob" module holding both core query executor iterator logic and over 1800 lines of tests. It made navigation and understanding the core operations difficult.
**Blueprint:** Refactored into a `src/query/executor/iterators/` module. Kept the core data definitions and implementation in `mod.rs` (reduced to ~1600 lines) and moved all tests to `tests.rs` (~1800 lines), maintaining the `#[cfg(test)]` block functionality but with better physical separation.
