...
## 2026-06-22 - Splitting Temporal Vector Index Blob
**Tangle:** `src/index/vector/temporal/mod.rs` was over 3700 lines long, a "Blob" module holding both core temporal indexing logic and over 2300 lines of tests. It made navigation and understanding the core operations difficult.
**Blueprint:** Extracted the `mod tests` block into a dedicated `src/index/vector/temporal/tests.rs` file. Kept the core data definitions and implementation in `mod.rs` (reduced to ~1400 lines) and moved all tests to `tests.rs` (~2300 lines), maintaining the `#[cfg(test)]` block functionality but with better physical separation.
