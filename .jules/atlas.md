...
**The Blob Anti-pattern in Tests**
**Tangle:** Massive source files (some over 3,000 lines) with inline `#[cfg(test)] mod tests { ... }` blocks containing thousands of lines of test code. This causes extreme bloat in core modules, degrading maintainability, readability, and increasing merge conflicts, blending test domain logic into core domain logic.
**Blueprint:** Extracted inline test blocks across 20+ core modules (e.g., `core/version.rs`, `query/planner.rs`, `storage/wal`) into dedicated `tests.rs` files within directory modules. Replaced the inline blocks with `#[cfg(test)] mod tests;` to enforce clean separation of concerns and reduce file sizes by up to 50%.
