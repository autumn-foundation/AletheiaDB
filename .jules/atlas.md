...
**Tangle:** Several modules (`src/mcp/server.rs`, `src/index/vector/temporal/mod.rs`) were still acting as "Blob" modules due to large inline `#[cfg(test)]` blocks. This made the core logic hard to navigate.
**Blueprint:** Extracted tests into separate `tests.rs` files within their respective module directories (`src/mcp/server/tests.rs` and `src/index/vector/temporal/tests.rs`). Updated `mod.rs` to include `#[cfg(test)] mod tests;`. This adheres to the architectural principle of separating test concerns from core implementation for large files.
