...
## 2026-05-26 - Splitting The Blob modules
**Tangle:** Several modules (`src/storage/checkpoint.rs`, `src/storage/migration.rs`, `src/index/vector/temporal/mod.rs`, and `src/query/executor/iterators.rs`) had grown into "Blob" modules containing both core implementations and thousands of lines of tests.
**Blueprint:** Refactored each of these files into a dedicated directory with a `mod.rs` for implementation and a `tests.rs` for unit tests. This physical separation significantly reduces module size and improves navigability.
