...
**[Weak Display Assertions]**
**Module:** `core::id`, `core::interning`, `core::property`, `core::temporal`, `core::vector`, `honeycomb::error`, `index::vector::distributed`, `query::lexer`, `storage::sharding::*`
**Severity:** 🟡 Suspect
**Finding:** A widespread pattern of weak testing for `fmt::Display` implementations. Numerous tests (e.g., `test_executor_error_display`, `test_migration_error_display`, `test_bitemporal_display`) verified the output merely by asserting that it `.contains()` a specific substring. This weak assertion allows mutants (e.g., accidental format string changes, missing parameters) to survive, providing false confidence that the formatting logic is correct.
**Evidence:** `cargo mutants` identified 27 surviving mutations specifically targeting `replace <impl fmt::Display for ...>::fmt` across multiple modules. The mutation replaced the actual `fmt` output with an empty or default string, but the tests passed or partially passed because they didn't assert the exact string output.
**Recommendation:** Refactored tests across all identified modules to use exact equality checks (`assert_eq!(format!("{}", err), "Exact Error String")`) or `.to_string()`. This eliminates the tautology and explicitly pins the formatting output contract.
