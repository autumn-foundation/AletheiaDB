...
**Temporal Logic Boundary Fixes**
**Module:** `src/core/temporal.rs`
**Summary:** Large number of mutants inside temporal testing boundaries `contains`, `is_closed`, `overlaps` and `contains_or_after` replaced boolean logic (`<` with `<=`, `==`, `>` and `||` with `&&`). Additionally there were mutant gaps in boundary behavior at `MAX_VALID_TIMESTAMP` and within serialization functions like exact length tests or `time::to_iso8601`.
**Diagnosis:** Weak assertions around exactly intersecting or exactly matching boundaries for intervals and single-point test paths. `MAX_VALID_TIMESTAMP` handling lacked explicit targeted boundary cases for `at()` and `from()`.
**Kill Shot:**
Added rigorous boundary logic test cases specifically targeting boundary equality testing, overlapping endpoints (e.g., `<` being distinguished cleanly from `<=`), serialization length verification (`deserialize_exact`), and panic logic on maximum valid timestamps. Introduced `test_sentinel_time_range_is_closed_boundary`, `test_sentinel_time_range_contains_or_after_boundary`, `test_sentinel_time_range_contains_range_exact`, and `test_sentinel_bitemporal_is_current_logic`.
