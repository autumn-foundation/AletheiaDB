...
**[Strengthen core ID bounds and generation tests]**
**Module:** core::id
**Summary:** Multiple surviving mutants replacing `+` with `-` or `/` in `MAX_VALID_ID`, as well as boundary off-by-one errors (`>` vs `==`) in ID generation logic, and loose `TryFrom` validation asserting on `Ok` instead of value.
**Diagnosis:** Weak tests - the test suite lacked precise boundary condition tests around `MAX_VALID_ID` and didn't strictly assert the actual exact return values for ID conversions and generator increments.
**Kill Shot:** Injected `test_id_generator_next_exact_boundaries`, `test_id_generator_ensure_at_least_exact_boundaries`, `test_tx_id_generator_next_exact_increment`, and specific exact-value checks for `TryFrom`, `FromStr`, and `Display`.
