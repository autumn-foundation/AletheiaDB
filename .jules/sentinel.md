...
**[Weak Test Coverage in Graph Models boundaries and specific values]**
**Module:** `src/core/graph.rs`
**Summary:** Mutants regarding logic replacements survived (replacing `==` with `!=` in `matches_label` and `has_label` / `has_label_str`), default value injections (returning `Default::default()` or `None` in `with_metadata`, `get_property`), and logic operator changes (replacing `&&` with `||` in `connects`).
**Diagnosis:** WEAK_TEST / MISSING_TEST - Existing tests asserted general functionality but were insufficiently targeted towards negative logical boundaries (e.g. explicitly tracking exact match false negative triggers, tracking exact non-null property retrievals instead of just Option checking, and specifically targeting explicit logical conditions for graph components `has_label` string and `connects` edge combinations).
**Kill Shot:** Appended explicit mutant-killing test blocks in `src/core/graph.rs` covering explicit true/false boundary cases, explicit properties `!PropertyValue::Null` assertions, explicit endpoint connections combinations, and explicitly constructed `with_metadata` instantiations verifying component preservation against defaults.
