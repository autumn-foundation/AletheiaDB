...
**[Weak Test Coverage in ID Generation and Parsing]**
**Module:** `src/core/id.rs`
**Summary:** Mutants regarding default returns inside `new_unchecked`, `as_u64`, and string formatting (`fmt::Display`, `FromStr`) survived across `NodeId`, `EdgeId`, `VersionId`, and `TxId`. Mutants mutating exact checks and boundaries in `IdGenerator::next`, `IdGenerator::current_approximate`, `IdGenerator::ensure_at_least`, and `TxIdGenerator` survived, as well as type conversion mismatches in `EntityId` casting.
**Diagnosis:** MISSING_TEST / WEAK_TEST - Existing tests asserted broad high-level generator uniqueness but missed exact boundary limits (like `MAX_VALID_ID`), default type parsing/formatting parity between ID structs and `EntityId` enum encapsulations.
**Kill Shot:** Appended explicit combinatorial exact assertions for boundaries, default formatting overrides, and generator sequence states via `kill_mutants_id` test directly inside `src/core/id.rs`.
