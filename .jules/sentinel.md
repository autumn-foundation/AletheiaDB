...
**[Weak Test Coverage in Graph Property & Label Checks]**
**Module:** `src/core/graph.rs`
**Summary:** Mutants regarding default returns (`true`/`false`/`None`) survived inside `has_label_str`, `has_label`, `get_property` across both `Node` and `Edge` structs. Mutants ignoring `with_metadata` metadata instantiation boundaries also slipped through.
**Diagnosis:** WEAK_TEST / MISSING_TEST - The previous test suite did not tightly bind negative constraints (asserting exact false conditions when wrong labels/properties were provided) nor explicitly check exact `PropertyMap` values down to the inner `PropertyValue` underlying types (`as_int`, `as_float`).
**Kill Shot:** Appended explicit exact property assertions `test_node_get_property_exact`, `test_edge_get_property_exact`, boundary condition checks `test_node_has_label_exact`, `test_edge_has_label_exact` alongside string matching tests directly inside `sentry_tests` of `src/core/graph.rs`.
