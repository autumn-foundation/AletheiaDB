...
**2026-02-16 - Unsafe Pointer Arithmetic in SIMD Functions (squared_diff_sum)**
**Threat:** The `squared_diff_sum_avx2` and `squared_diff_sum_sse2` functions in `src/core/vector/simd.rs` still used manual pointer arithmetic (`ptr.add(offset)`) inside the main processing loop, unlike other SIMD functions which had been hardened. This remained a fragility risk where logic errors could lead to buffer over-reads and Undefined Behavior (UB).
**Defense:** Refactored `squared_diff_sum_avx2` and `squared_diff_sum_sse2` to use `chunks_exact` and `zip` iterator combinators, matching the safety pattern of `dot_and_magnitudes`. This removes manual pointer arithmetic and improves safety maintainability. Added `test_simd_squared_diff_sum_large_vector` and `test_simd_squared_diff_sum_unaligned` to `src/core/vector/sentry_tests.rs` to verify correctness and robustness.
