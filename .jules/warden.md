...
**2026-06-15 - Deserialization Bombs (HTTP Server)**
**Threat:** The Actix-web server configuration in `src/http/server.rs` (`configure_app`) did not explicitly limit JSON payload sizes using `web::JsonConfig`. This omission allows an attacker to send massive JSON payloads (e.g., extremely large `ExecuteQuery` strings or `properties` objects) that `actix-web` will attempt to deserialize in memory. This constitutes an unbound memory allocation vector, creating a significant Denial of Service (DoS) risk via Out of Memory (OOM) crashes or CPU starvation.
**Defense:** Explicitly added `web::JsonConfig::default().limit(2 * 1024 * 1024)` via `.app_data()` in `configure_app`. This enforces a strict 2MB limit on incoming JSON payloads at the framework level, rejecting oversized requests with a `413 Payload Too Large` error before any application logic or deserialization begins, satisfying the "Defense in depth" and "Parse, don't validate" mandates.
