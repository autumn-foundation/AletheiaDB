...
**2026-02-16 - Deep Pagination DoS in FindNeighbors**
**Threat:** The `FindNeighbors` HTTP endpoint lacked validation for `offset` and `limit` parameters. A malicious actor could provide a massive `offset` (e.g., `usize::MAX`) combined with a small `limit`. The summation `offset + limit` could overflow `usize` (wrapping to a small number), bypassing checks. Even without overflow, a huge `offset` forces the iterator to skip billions of edges, causing CPU exhaustion (DoS).
**Defense:** Added a strict check `offset.saturating_add(limit) <= 10,000` in `src/http/handlers.rs`. This prevents both integer overflow attacks (via saturation) and deep pagination DoS. Verified with `test_warden_find_neighbors_overflow`.
