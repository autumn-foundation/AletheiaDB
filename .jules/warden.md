...
**2026-02-16 - Unbounded HNSW Index Dimensions**
**Threat:** The `HnswIndexBuilder` and `HnswIndex::load` methods accepted a `dimensions` parameter without enforcing the `MAX_VECTOR_DIMENSIONS` limit (100,000). A malicious user could request an index with excessive dimensions (e.g., 1 billion), causing `usearch` to attempt massive memory allocations or potentially leading to Undefined Behavior in FFI callbacks if `usearch` mishandled the size.
**Defense:** Implemented explicit validation in `HnswIndexBuilder::build`, `HnswIndex::load`, and `HnswIndex::open_mmap` to verify that `dimensions <= MAX_VECTOR_DIMENSIONS`. Added `tests/warden_hnsw_dos.rs` to verify that massive dimension requests are rejected with a specific `InvalidVector` error rather than triggering OOM.
