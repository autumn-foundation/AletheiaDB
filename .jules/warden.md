...
**2026-02-15 - FFI Boundary Panic Safety in HNSW Custom Metrics**
**Threat:** The `create_metric_wrapper` function in `src/index/vector/hnsw.rs` constructed a callback passed to the C++ `usearch` library. This callback could panic (due to invalid inputs or user code), causing unwinding across FFI boundaries, which is Undefined Behavior (UB) and typically aborts the process.
**Defense:** Wrapped the metric calculation logic in `std::panic::catch_unwind`. Instead of panicking on null/unaligned pointers or user logic errors, the callback now safely catches the unwind and returns `f32::MAX` (a large distance), ensuring the search continues gracefully without crashing the host process. Updated tests to assert this fallback behavior instead of expecting panics.
