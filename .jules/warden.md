...
**2026-02-16 - DoS Vulnerability in FindNode/FindNeighbors**
**Threat:** The `FindNode` and `FindNeighbors` endpoints allowed users to request up to 10,000 items (via implicit pagination logic) or uncapped limits (in `FindNode`). This could lead to excessive memory allocation (`Vec::with_capacity(limit)`) and CPU usage (large scans/filtering) for a single request. While 10,000 is not huge, concurrent requests could exhaust server memory.
**Defense:** Introduced `MAX_QUERY_LIMIT` (1000) in `src/http/handlers.rs`. Enforced this limit on `limit` parameters for both `FindNode` and `FindNeighbors` using `.min(MAX_QUERY_LIMIT)`. This ensures that no single request can retrieve more than 1000 items, significantly reducing the memory footprint per request.
**Verification:** Added `src/http/warden_tests.rs` (integrated into `src/http/mod.rs`) which verifies that a request with `limit=1500` returns at most 1000 items. Verified that existing tests still pass.
