...
**[Hybrid Logical Clocks False Confidence and Serialization Coverage]**
**Learning:** `cargo mutants` run against `src/core/hlc.rs` revealed surviving mutants hidden behind basic functionality tests. Mathematical operations (`as_secs`, `as_millis`, `increment_logical`), buffer logic (`deserialize`), and structural comparison (`receive` replacing self with `Default::default()`) survived because tests often checked broad conditions rather than exact behaviors on precise return values and boundary conditions.
**Action:** When testing foundational components like time or logic blocks, always explicitly assert on both exact numerical correctness across ranges (e.g., negative values for timestamp logic, exact byte structure for serialization checks) and strict boundary rejection logic (like rejecting logical components on `i64::MAX` sentinel time boundaries) instead of just checking `.is_ok()`.
