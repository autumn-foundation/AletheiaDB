...
**Limit Pushdown AST Coverage**
**Learning:** `cargo mutants` revealed missing test coverage for limit pushdown logic across `LogicalPlan` nodes. Specifically, the boolean short-circuit logic `changed || effective_limit != *n` in `UnaryOp::Limit` and `UnaryOp::VectorRank` wasn't exercised when limits were propagated down through operators like `Filter`, `Sort`, or `Project`. Tests simply merged consecutive limits, masking the boolean logic of limits that passed through nodes unaffected.
**Action:** Always test planner rule optimizations traversing through different intermediate `LogicalOp` nodes, not just directly adjacent nodes, to ensure boundary variables (like limits) and recursive return flags (like `changed` booleans) are correctly propagated and evaluated by the host AST node.
