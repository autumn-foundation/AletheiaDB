...
**PropertyValue PartialEq Explicit Variant Testing**
**Learning:** `cargo mutants` reveals that enum `PartialEq` derivations or manual implementations often miss testing the inequality paths across different variants (the `_ => false` arm), leaving them vulnerable to regression where any two variants could wrongly evaluate to `true`. Similarly, pointer equality shortcuts (`Arc::ptr_eq`) and deep equality shortcuts might be mutated without test failure if explicit permutations aren't checked.
**Action:** Always write cross-variant inequality assertions (e.g. `assert_ne!(PropertyValue::Null, PropertyValue::Bool(true))`) in addition to same-variant tests. Provide explicit type annotations for `Arc` instantiations (e.g., `let s: Arc<str> = ...`) to avoid type inference issues during test compilation.
