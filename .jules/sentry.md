...
## Unreachable Panic Risks
**Learning:** `unreachable!()` calls in Rust, particularly in generic matching blocks or parsers, act exactly like panics in production. A number of `unreachable!` statements were used in parsers (like the temporal SQL parser) and query conversion steps for unrecognized tokens, or in `match` branches where certain enum variants were considered logically impossible. These were identified by explicitly scanning for them. If these paths were accidentally triggered (e.g., via corrupt AST nodes), they would cause the system to crash. While some are structurally impossible to hit and thus safe (like in iterator retry loops), those exposed to complex `match` expressions required test coverage to ensure they behave predictably.
**Action:** Audit codebase for `unreachable!()`, converting them to explicit errors where possible. For branches that are truly unreachable but must be retained for exhaustiveness, explicitly write `#[should_panic]` unit tests using artificially constructed malicious inputs (e.g., passing invalid enum variants directly to internal functions) to guarantee that if the logic ever shifts to make them reachable, the test suite acts as an immediate tripwire.
