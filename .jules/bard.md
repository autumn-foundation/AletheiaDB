...
## 2025-03-05 - Doctest Dependencies and Node Creation
**Confusion:** Doctests in experimental features (, , , , , , ) were using hardcoded mock NodeIds (like ) or casting integers (), which didn't satisfy trait bounds or didn't actually exist when queried, leading to  runtime errors or compile errors. Additionally,  caused subtraction trait bound errors with .
**Clarification:** You must explicitly create nodes within the doctest using . Do not forget to add  and potentially  depending on the test requirements. Use  for temporal arithmetic.

## 2025-03-05 - Doctest Dependencies and Node Creation
**Confusion:** Doctests in experimental features (aura, chameleon, fossil, janus, ripple, tremor, synergy) were using hardcoded mock NodeIds (like `NodeId::new(0).unwrap()`) or casting integers (`123.into()`), which didn't satisfy trait bounds or didn't actually exist when queried, leading to NodeNotFound runtime errors or compile errors. Additionally, `time::now() - integer` caused subtraction trait bound errors with HybridTimestamp.
**Clarification:** You must explicitly create nodes within the doctest using `db.write(|tx| tx.create_node("Label", Default::default()))?`. Do not forget to add `# use aletheiadb::api::transaction::WriteOps;` and potentially `# use aletheiadb::core::property::PropertyMapBuilder;` depending on the test requirements. Use `time::from_secs(time::now().wallclock() / 1_000_000 - duration_secs)` for temporal arithmetic.
