...
## 2025-04-22 - Broken Links in Feature-Gated Modules
**Confusion:** The experimental modules table in `src/experimental/mod.rs` tried to link to feature-gated modules (`reasoning`, `temporal`, `diagnostics`, `characterization`) and `semantic_search` which was also not resolved correctly. Since these are optional features, `cargo doc` fails with `rustdoc::broken_intra_doc_links` warnings when the features are disabled or the path is otherwise incorrect.
**Clarification:** To prevent `rustdoc::broken_intra_doc_links` warnings in Rust documentation for modules that are feature-gated, use standard markdown backticks (`` `item` ``) instead of intra-doc links (`[item]`) when referencing modules, crates, or items that might not exist in the resolved module tree during default `cargo doc` generation.
