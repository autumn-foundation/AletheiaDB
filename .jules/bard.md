...
## 2025-03-05 - Core Graph Elements Current State
**Confusion:** The `Node` and `Edge` structs in `src/core/graph.rs` lacked extensive documentation explaining that they represent the *current* state of the graph, causing confusion for users who expected to find historical data natively attached to these structures. The rationale behind using `InternedString` for labels to optimize the hot path and memory efficiency was also not prominently featured or demonstrated via clear examples.
**Clarification:** Added comprehensive `# Context`, `# Usage`, `# Details`, and `## Examples` sections to `Node`, `Edge`, and their associated methods. Clarified that temporal versions reside elsewhere, provided copy-pasteable examples for creating and querying these structures, and detailed why interning is critical for performance.
