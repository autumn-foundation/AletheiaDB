...
**Cut:** Deleted the `GraphView` trait and `src/db/graph_view.rs` module. Refactored all consumers (`hybrid.rs` and `semantic_pathfinding.rs`) to use the concrete `AletheiaDB` struct directly.
**Saved:** ~100 lines of boilerplate (trait definition, mock implementations, explicit dereferences) + removed dynamic dispatch overhead.
