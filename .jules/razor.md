...
**Cut:** Deleted the `GraphView` trait, `query::traits` module, and `db::graph_view` module. Refactored all consumers in `query::hybrid` and `query::semantic_pathfinding` to use the concrete `AletheiaDB` struct directly.
**Saved:** ~250 lines of boilerplate (trait definitions, mock implementations, duplicate imports) + removed generic monomorphization/dynamic dispatch overhead.
