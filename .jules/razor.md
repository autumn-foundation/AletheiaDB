...
## [Reduction]
**Bloat:** The "One-Time" Trait
**Cut:** Removed the `Resonator` trait which was only implemented by `ActivityDensityResonator`. Transformed it into a concrete method on the struct.
**Saved:** 5 lines of code, 1 layer of pointer indirection (removed `Box<dyn Resonator>`), and cognitive load of an unnecessary abstraction.
