...
**Eliminate intermediate Vec<Row> collection**
**Learning:** Collecting iterators into intermediate `Vec<T>` structures before structurally mapping them (e.g. into columns) causes unnecessary `O(N)` heap allocations, particularly for query results. Additionally, lazy initialization (using `Option<Vec<V>>` and `resize_with`) of columnar vectors correctly aligns memory only if padded accurately against actual present entity counts instead of total loop indices.
**Action:** When mapping rows to columns, use a single-pass loop directly on the iterator (`while let Some(row) = iter.next()`). If a column is missing for early rows but appears later, use `Option<Vec<T>>` to lazily allocate it when first encountered, and pad it with default values up to the *number of valid entities seen so far* (e.g., `nodes.len() - 1`), rather than the global loop index, to avoid data misalignment.
