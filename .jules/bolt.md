...
**[Optimize intermediate Vec allocation in match arms]**
**Learning:** Found multiple instances where `.filter_map().collect::<Vec<_>>()` was used to dynamically collect items, but we can replace it with `.with_capacity()` along with an exact pre-calculation of capacity based on `.size_hint()` of the iterator. However, since the elements need to be verified at runtime (e.g. `edge_visible_at_time`), it's better to instantiate the iterators once, get the size hint for pre-allocation with `Vec::with_capacity()`, and use a `for` loop to filter out items without using an intermediate vector and collect. Also, `clippy` pointed out `if` statements can be collapsed (e.g. using `&& let`).
**Action:** Replace `.filter_map().collect()` sequences with `.with_capacity(iter.size_hint().0)` and a `for` loop over the iterator to directly populate the pre-allocated vector. Use `&& let` to collapse `if` blocks for more idiomatic Rust code when combining conditions and pattern matching.
