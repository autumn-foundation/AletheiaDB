...
**2026-11-20 - Identity Hashing for Sparse Vector Index DashMap**\n**Learning:** The Sparse Vector Index uses `DashMap` for `inverted_index` (key: `u32`), `vectors` (key: `NodeId`), and `doc_freq` (key: `u32`). By default, `DashMap` uses SipHash, which incurs unnecessary hashing overhead for simple integer keys.\n**Action:** Replace `DashMap::with_capacity` with `DashMap::with_capacity_and_hasher(capacity, BuildHasherDefault::<IdentityHasher>::default())` for integer-keyed maps in hot paths. This eliminates SipHash overhead, improving insertion and lookup performance.
