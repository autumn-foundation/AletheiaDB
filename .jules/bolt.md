...
**[Optimize QueryResults::collect_structured Single-Pass Allocation]**
**Learning:** `collect_structured` previously iterated twice over `QueryResults` to construct `QueryResult`, creating a massive intermediate `Vec<QueryRow>` that drastically increased heap allocation overhead. Additionally, missing `paths` vectors were erroneously padded with `vec![]`, risking type-mismatches.
**Action:** Implemented a single-pass extraction loop in `collect_structured`. Dynamically allocated and padded (`resize_with(idx, Default::default)`) columnar optional vectors for `properties`, `scores`, `paths`, and `versions` on-the-fly. This successfully eliminated the large intermediate `Vec<QueryRow>` array allocation and greatly lowered heap footprint per structured result extraction.
