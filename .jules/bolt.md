...
**[Optimize physics loops avoiding intermediate vector allocs]**
**Learning:** In Rust, calling `.collect()` into intermediate vectors when computing multi-pass formulas (e.g., relative vectors, dot products, point at time `t`) forces unnecessary heap allocations. Using single-pass loops zipping iterator items to calculate dot products and minimum distances removes these allocations. Also, calling methods like `.sqrt()` on untyped floating variables leads to ambiguous type errors (E0689).
**Action:** When computing properties like `distance` or `dot product` from multiple arrays, use `for (a, b) in array_a.iter().zip(array_b.iter())` to calculate these inline and accumulate them in mutable floats, eliminating O(N) allocations for temporary vectors. Additionally, ensure float accumulators are explicitly typed (e.g., `let mut val: f32 = 0.0;`) to avoid compiler errors.
