...
**Learning:** Replaced manual iterator combinations for vector math (e.g. `zip().map().sum()`, `zip().map().powi(2).sum()`) with SIMD-accelerated methods from `crate::core::vector`. Found that although replacing the logic in `ariadne.rs` with `cosine_similarity` passed the initial test, the vectors were assumed to be already normalized. Using `cosine_similarity` adds unnecessary division and square root operations. For normalized vectors, `dot_product` provides the same mathematical result faster and cheaper. In Rust, standard `///` doc comments on `let` bindings will trigger `clippy::unused_doc_comments` warnings in strict modes.
**Action:** Use `crate::core::vector` methods for SIMD-accelerated vector math operations to avoid iterator allocations and missing AVX2 extensions. When working with guaranteed normalized vectors, explicitly use `dot_product` instead of `cosine_similarity` to maximize efficiency. Use `//` for inline code comments to avoid triggering `unused_doc_comments`.
