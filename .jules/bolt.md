...
**[Zero-Allocation SQL Keyword Parsing]**
**Learning:** In string parsers (like `find_keyword_outside_strings`), calling `.to_uppercase()` creates a complete owned copy of the string. Storing characters via `.chars().collect::<Vec<char>>()` adds another two full string-length heap allocations per function call. Slicing with `.iter().collect::<String>()` creates an `O(N)` number of intermediate loop allocations.
**Action:** Achieve zero-allocation parsing by iterating over `char_indices().peekable()` to track string literal scopes. For ASCII-dominant keywords, compare string slices using `remainder[..len].eq_ignore_ascii_case(keyword)`, which requires zero heap allocations. Use `.starts_with()` on `.to_uppercase()` as a fallback only when dealing with non-ASCII boundaries.
