...
// ════════════════════════════════════════════════════════════════════════════
// (1) MCP catalog == inventory — EXACT set equality across all 52 tools.
// ════════════════════════════════════════════════════════════════════════════
...
async fn mcp_catalog_equals_inventory_exactly() {
    ...
    assert_eq!(
        inv.len(),
        52,
        "inventory must advertise exactly 52 MCP tools"
    );
    ...
    assert_eq!(
        live.len(),
        52,
        "the live catalog must be exactly 52 tools (got {})",
        live.len()
    );
}

// ════════════════════════════════════════════════════════════════════════════
// (2) Access-class conformance for all 52 (belt-and-suspenders full-set check;
...
#[tokio::test]
async fn access_class_conformance_for_all_52() {
    let (db, store) = fixture();
    let client = build_server_client(db, store, AuthMode::Required);
    let live = live_mcp_catalog(&client).await;
...
async fn budgetable_and_cursorable_sets_match_inventory() {
    ...
    assert_eq!(
        inv_budgetable.len(),
        19,
        "inventory must mark exactly 19 budgetable read tools"
    );
    // Cross-check against the totals block too.
    assert_eq!(
        doc["totals"]["mcp_budgetable_read_tools"].as_u64(),
        Some(19),
        "totals.mcp_budgetable_read_tools must be 19"
    );
