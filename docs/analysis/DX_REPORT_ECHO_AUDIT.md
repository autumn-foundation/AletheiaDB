...
### 3. NarrativeGenerator Documentation is Misleading (New Finding)

The `NarrativeGenerator` struct (part of the experimental `nova` features) is exposed even when the `nova` feature is disabled, via a stub implementation that panics at runtime. This is intended to provide helpful error messages.

**The Problem:** The documentation for this stub (generated by `cargo doc`) does not warn the user that it panics. It looks like a valid API. A user browsing docs locally without features enabled would be confused.

**The Fix:** Update the doc comments in `src/experimental/mod.rs` to explicitly warn that the `nova` feature is required and that the stub methods panic.

## Action Items

- [ ] Update `README.md` Tiered Storage example to use `AletheiaDBConfig`.
- [ ] (Optional) improve Narrative Generation example comments.
- [x] Update `src/experimental/mod.rs` doc comments for `NarrativeGenerator` stub.
