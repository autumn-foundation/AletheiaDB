...
| **Replica read concurrent with an in-flight apply** | `tests/replication_engine.rs::torn_frame_safety_never_exposes_a_partial_transaction`, `::torn_frame_safety_holds_under_sustained_concurrent_load` (Issue #3788) | A transport capped at 1-2 entries per fetch splits every `[BeginTx .. CommitTx]` band across polls, so the applier is publishing near-continuously while 4 reader threads hammer the replica's point lookups | A read never observes a partially-applied transaction: once the frame's first node is visible, its second node and its edge -- read strictly later -- are visible too (`(visible, missing)` is the torn shape and must never occur) | `cargo test --test replication_engine torn_frame_safety` |
