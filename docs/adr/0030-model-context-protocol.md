...
GallifreyDB's primary use case is enabling "Reasoning LLMs" to query knowledge graphs with temporal context.
Currently, integrating GallifreyDB with an LLM agent (like Claude, ChatGPT, or custom LangChain agents) requires developers to write custom glue code:
1.  Define tool schemas (JSON) manually.
2.  Implement a REST or Python wrapper to translate tool calls to GallifreyDB API calls.
...
We will implement a **Native MCP Server** directly within the GallifreyDB codebase (`src/mcp`).

The implementation will:
1.  **Be Built-in**: Not a separate sidecar, but a feature of the main binary (or a dedicated `gallifrey-mcp` binary sharing the same core).
...
*   **Zero-Config Integration**: Users can add GallifreyDB to Claude/Cursor config by simply pointing to the binary. No Python glue code needed.
