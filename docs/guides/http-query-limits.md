...
| Engine-level cancellation of in-flight CPU work | ✅ | ✅ | ✅ | **Landed (engine lane)** — cooperative row-granular cancellation in the executor; see [query-resource-limits.md](query-resource-limits.md). The MCP `query`-tool worker now self-cancels near its deadline; the HTTP handler's `tokio::timeout` is cooperative when the request routes through the engine guard |
| Query **memory budget** | — | ✅ (MCP `query` tool, default-off) | ✅ (Rust builder/config) | **Landed (engine lane)** — `estimate_row_bytes` working-memory proxy with default/override/ceiling; HTTP-surface parity is a follow-up. See [query-resource-limits.md](query-resource-limits.md) |
