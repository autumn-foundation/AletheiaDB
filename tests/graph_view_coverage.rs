...
fn test_graph_view_trait_coverage() {
    ...
    // get_node_at_time
    let n1_t = db.get_node_at_time(node1, now, now).unwrap();
    assert_eq!(n1_t.id, node1);

    // get_edge_at_time
    let e_t = db.get_edge_at_time(edge_id, now, now).unwrap();
    assert_eq!(e_t.id, edge_id);

    // get_outgoing_edges_at_time
    let out_t = db.get_outgoing_edges_at_time(node1, now, now);
    assert_eq!(out_t.len(), 1);

    // get_incoming_edges_at_time (Uncovered Lines 64-71)
    let in_t = db.get_incoming_edges_at_time(node2, now, now);
