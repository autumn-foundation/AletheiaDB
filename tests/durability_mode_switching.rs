use gallifreydb::{GallifreyDB, DurabilityMode, WriteOps};
...
fn test_switch_to_async() {
    ...
    db.write(|tx| {
        tx.create_node("Test", Default::default())
    }).unwrap();
}
...
fn test_transaction_captures_mode_at_start() {
    ...
    db.write(|tx| {
        tx.create_node("Test2", Default::default())
    }).unwrap();
}
...
fn test_graceful_group_commit_transition() {
    ...
    println!("Commit took {:?}, Switch took {:?}", commit_elapsed, switch_elapsed);

    // Switch should have triggered immediate flush
    assert!(commit_elapsed < Duration::from_millis(1500), "Commit took too long: {:?}", commit_elapsed);
...
fn test_set_durability_mode_validated() {
    ...
    // Valid mode
    db.set_durability_mode_validated(DurabilityMode::group_commit_validated(10, 200)).unwrap();
    assert!(matches!(db.default_durability(), DurabilityMode::GroupCommit { .. }));

    // Invalid mode (validation should fail)
    let result = db.set_durability_mode_validated(DurabilityMode::group_commit_validated(1001, 200));
