...
mod tests {
    ...
    #[test]
    fn test_max_drift_constants_exact_math() {
        // Targets:
    ...
    fn test_hybrid_timestamp_receive_exact_wallclock_logic() {
        ...
        assert_eq!(next1.logical(), 0); // physical won

        // `else if new_wallclock == self.wallclock && new_wallclock == msg.wallclock` -> max logical + 1
        ...
        assert_eq!(next2.logical(), 21); // match both

        // `else if new_wallclock == self.wallclock` -> local + 1
        ...
        assert_eq!(next3.logical(), 11); // matched local

        // `else` (msg wallclock was chosen) -> msg + 1
        ...
        assert_eq!(next4.logical(), 21); // matched msg

        // Target specifically the mutant `if new_wallclock == self.wallclock || new_wallclock == msg.wallclock`
        // We need a case where ONE is true, but NOT BOTH.
        // If the mutant is active, `||` will incorrectly drop into the `match both -> max logical + 1` logic.
        // Let local=2000, msg=1000, phy=2000 -> new_wallclock=2000.
        // `new == local (2000==2000) T` AND `new == msg (2000==1000) F`.
