...
mod sentry_tests {
    ...
    fn test_bitemporal_methods() {
        ...
        let tx_start = HybridTimestamp::new(150, 0).unwrap();
        ...
        assert!(with_valid.is_valid_at(HybridTimestamp::new(100, 0).unwrap()));
        assert!(!with_valid.is_valid_at(HybridTimestamp::new(50, 0).unwrap()));
    ...
    #[test]
    fn test_bitemporal_close_mutants() {
        let bti = BiTemporalInterval::now(100.into(), 100.into());
    ...
    fn test_timerange_serialize_mutants() {
        ...
        // Mutants returning vec![], vec![0], vec![1] should fail since len != 24
        assert_eq!(bytes.len(), 24);

        // We can test deserialization returns same value
        let (deserialized, size) = TimeRange::deserialize(&bytes).unwrap();
        assert_eq!(size, 24);
        assert_eq!(deserialized.start(), Timestamp::from(100));
        assert_eq!(deserialized.end(), Timestamp::from(200));
    }
