...
mod additional_sentry_tests {
    ...
    fn test_sentry_is_currently_valid_and_recorded() {
        ...
        let interval_closed_valid =
            BiTemporalInterval::new(TimeRange::new(start, end).unwrap(), TimeRange::from(start));
        ...
        let interval_closed_tx =
            BiTemporalInterval::new(TimeRange::from(start), TimeRange::new(start, end).unwrap());
    ...
    fn test_sentry_timerange_overlaps_mutants() {
        ...
        assert!(
            !empty.overlaps(&non_empty),
            "Empty range should not overlap (kills || to && mutant)"
        );
        assert!(
            !non_empty.overlaps(&empty),
            "Empty range should not overlap (kills || to && mutant)"
        );
    }
}
