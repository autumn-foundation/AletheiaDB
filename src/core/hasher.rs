    ...
    fn write(&mut self, bytes: &[u8]) {
        ...
        match bytes.len() {
            ...
            2 => {
                ...
                let arr: [u8; 2] = bytes.try_into().expect("length matched 2");
                self.update_state(u16::from_le_bytes(arr) as u64);
            }
            4 => {
                // SAFETY: length checked
                let arr: [u8; 4] = bytes.try_into().expect("length matched 4");
                self.update_state(u32::from_le_bytes(arr) as u64);
            }
            8 => {
                // SAFETY: length checked
                let arr: [u8; 8] = bytes.try_into().expect("length matched 8");
            ...
            16 => {
                ...
                let low_arr: [u8; 8] = bytes[0..8].try_into().expect("length matched 8");
                let high_arr: [u8; 8] = bytes[8..16].try_into().expect("length matched 8");
