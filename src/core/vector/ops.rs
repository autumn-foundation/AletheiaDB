...
pub fn normalize(v: &[f32]) -> Vec<f32> {
    ...
    // We slice to v.len() to ensure length match for scale_and_copy assertion.
    let dst = &mut result.spare_capacity_mut()[..v.len()];
    scale_and_copy(v, dst, inv_mag);

    // SAFETY: scale_and_copy guarantees that all elements in the slice are written.
    // We ensured the slice length is exactly v.len().
