...
mod tests {
    ...
    fn test_scale_and_copy_implementation_coverage() {
        ...
        unsafe fn as_uninit_mut(s: &mut [f32]) -> &mut [MaybeUninit<f32>] {
            // SAFETY: s is a valid mutable slice. MaybeUninit<f32> has same layout as f32.
            unsafe {
                std::slice::from_raw_parts_mut(s.as_mut_ptr() as *mut MaybeUninit<f32>, s.len())
            }
        }

        // 1. Unconditional Scalar coverage
        scale_and_copy_scalar(&src, unsafe { as_uninit_mut(&mut dst) }, scalar);
