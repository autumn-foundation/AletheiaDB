...
mod tests {
    ...
    fn test_scale_and_copy_implementation_coverage() {
        ...
        let mut dst: Vec<f32> = Vec::with_capacity(17);
        let scalar = 2.0;
        let expected = vec![2.0f32; 17];

        // 1. Unconditional Scalar coverage
        scale_and_copy_scalar(&src, dst.spare_capacity_mut(), scalar);
        ...
        #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
        {
            ...
            if is_x86_feature_detected!("sse2") {
                unsafe { x86_ops::scale_and_copy_sse2(&src, dst.spare_capacity_mut(), scalar) };
                unsafe { dst.set_len(17) };
            ...
            if is_x86_feature_detected!("avx2") {
                unsafe { x86_ops::scale_and_copy_avx2(&src, dst.spare_capacity_mut(), scalar) };
                unsafe { dst.set_len(17) };
