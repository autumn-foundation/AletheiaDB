...
use super::*;
    ...
    fn test_property_map_builder() {
        ...
        assert_eq!(map.len(), 3);
        assert_eq!(map.get("name").and_then(|v: &PropertyValue| v.as_str()), Some("Alice"));
        assert_eq!(map.get("age").and_then(|v: &PropertyValue| v.as_int()), Some(30));
    ...
    fn test_property_map_copy_on_write() {
        ...
        assert_ne!(map1, map2);
        assert_eq!(map1.get("key").and_then(|v: &PropertyValue| v.as_str()), Some("value1"));
        assert_eq!(map2.get("key").and_then(|v: &PropertyValue| v.as_str()), Some("value2"));
    }
    ...
    fn test_vector_in_property_map() {
        ...
        assert_eq!(map.len(), 2);
        assert_eq!(
            map.get("embedding").and_then(|v: &PropertyValue| v.as_vector()),
            Some(&embedding[..])
        );
    }
    ...
    fn test_serialize_property_map_round_trip() {
        ...
        assert_eq!(
            deserialized.get("name").and_then(|v: &PropertyValue| v.as_str()),
            Some("Alice")
        );
        assert_eq!(deserialized.get("age").and_then(|v: &PropertyValue| v.as_int()), Some(30));
        assert_eq!(
            deserialized.get("active").and_then(|v| v.as_bool()),
            Some(true)
        );
        assert_eq!(
            deserialized.get("score").and_then(|v: &PropertyValue| v.as_float()),
            Some(98.5)
        );
    }
    ...
    fn test_serialize_property_map_with_vector() {
        ...
        assert_eq!(
            deserialized.get("embedding").and_then(|v: &PropertyValue| v.as_vector()),
            Some(&embedding[..])
        );
    }
        ...
        let handles: Vec<_> = (0..10)
            .map(|_| {
                ...
                thread::spawn(move || {
                    ...
                    for _ in 0..100 {
                        assert_eq!(
                            map_clone.get("shared_key").and_then(|v: &PropertyValue| v.as_str()),
                            Some("value")
                        );
    ...
    fn test_property_key_get_efficiency() {
        ...
        assert_eq!(value1.and_then(|v: &PropertyValue| v.as_str()), Some("value"));

        // get_by_interned_key() should be more efficient for repeated lookups
        let value2 = map.get_by_interned_key(&interned_key);
        assert_eq!(value2.and_then(|v: &PropertyValue| v.as_str()), Some("value"));
    ...
    fn test_serialize_sparse_vector_in_property_map() {
        ...
        assert_eq!(deserialized.len(), 2);
        assert_eq!(deserialized.get("id").and_then(|v: &PropertyValue| v.as_int()), Some(123));
        ...
        let handles: Vec<_> = (0..10)
            .map(|i| {
                thread::spawn(move || {
                    ...
                    assert_eq!(map.len(), 3);
                    assert_eq!(
                        map.get("shared_key_1").and_then(|v: &PropertyValue| v.as_str()),
                        Some("value1")
                    );
                    assert_eq!(
                        map.get(&unique_key).and_then(|v: &PropertyValue| v.as_int()),
                        Some(i as i64)
                    );
                })
            })
    ...
    fn test_from_iter_duplicate_keys() {
        ...
        assert_eq!(map.get("key").and_then(|v: &PropertyValue| v.as_int()), Some(2));
    }
