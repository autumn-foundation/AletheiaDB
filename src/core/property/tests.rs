...
use crate::core::property::serialization::{serialize_vector_into, serialize_sparse_vector_into};
...
#[test]
fn test_serialize_vector_into_coverage() {
    let mut buffer = Vec::new();
...
#[test]
fn test_serialize_sparse_vector_into_coverage() {
    let mut buffer = Vec::new();
    let sparse = crate::core::vector::SparseVec::new(vec![0], vec![1.0], 5).unwrap();
    serialize_sparse_vector_into(&sparse, &mut buffer);
    let (deserialized, _) = deserialize_sparse_vector(&buffer).unwrap();
    assert_eq!(deserialized.nnz(), sparse.nnz());
}
