...
impl EntityTimelines {
    ...
    /// An invalid index indicates internal inconsistency, so it falls back gracefully
    /// using `.unwrap_or_else` avoiding panics in edge cases while still failing loudly in dev.
    #[inline]
    fn resolve_version_id(&self, index: VersionMetadataIndex) -> Option<VersionId> {
    ...
    fn resolve_version_ids_iter<'a>(
        ...
        indices.iter().filter_map(|&idx| {
            let res = self.resolve_version_id(idx);
            debug_assert!(res.is_some(), "internal error: invalid metadata index");
            res
        })
    }
