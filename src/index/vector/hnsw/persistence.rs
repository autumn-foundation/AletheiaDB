...
pub(crate) fn load_mappings_with_integrity(
    ...
    let (count, metadata, header_overhead) = match version {
        ...
        2 => {
            ...
            let dims = u64::from_le_bytes(buf[0..8].try_into().map_err(|e| {
                Error::Vector(VectorError::IndexError(format!(
                    "Failed to parse dimensions from header: {}",
                    e
                )))
            })?) as usize;
            let quant = Quantization::from_u8(buf[8])?;
            let metric = DistanceMetric::from_u8(buf[9])?;
            let count = u64::from_le_bytes(buf[10..18].try_into().map_err(|e| {
                Error::Vector(VectorError::IndexError(format!(
                    "Failed to parse count from header: {}",
                    e
                )))
            })?) as usize;
    ...
    while remaining_entries > 0 {
        ...
        for chunk in slice.chunks_exact(16) {
            let node_id_raw = u64::from_le_bytes(chunk[0..8].try_into().map_err(|e| {
                Error::Vector(VectorError::IndexError(format!(
                    "Failed to parse node_id from mappings data: {}",
                    e
                )))
            })?);
            let key = u64::from_le_bytes(chunk[8..16].try_into().map_err(|e| {
                Error::Vector(VectorError::IndexError(format!(
                    "Failed to parse key from mappings data: {}",
                    e
                )))
            })?);
...
pub(crate) fn verify_index_header(
    ...
    let vector_byte_size = u32::from_le_bytes(header[4..8].try_into().map_err(|e| {
        Error::Vector(VectorError::IndexError(format!(
            "Failed to parse vector_byte_size from index header: {}",
            e
        )))
    })?) as usize;
