...
mod consistency_tests {
    ...
    #[test]
    fn test_concurrent_add_and_save() {
        let dir = tempfile::tempdir().unwrap();
        ...
        // Writer thread: continuously adds new vectors (Vacant path)
        ...
        let saver_handle = thread::spawn(move || {
            while saver_running.load(Ordering::Relaxed) {
                if let Err(e) = saver_index.save(&saver_path) {
                    // Ignore errors during shutdown race
        ...
        // Verify consistency using internal mapping count
    ...
    fn test_add_race_condition_vacant() {
        ...
        // All threads try to add the SAME new node ID concurrently
        for _ in 0..threads {
            let index = index.clone();
            handles.push(thread::spawn(move || {
                // Return result to check if we hit the race error
        ...
        if race_detected {
            println!("Successfully triggered Vacant path race condition!");
        }
    ...
    fn test_add_race_condition_occupied() {
        ...
        // All threads try to UPDATE the same node ID concurrently
        for i in 0..threads {
            let index = index.clone();
            let b = barrier.clone();
            handles.push(thread::spawn(move || {
                b.wait();
                // Vary vector slightly
                let val = i as f32 / 10.0;
                index.add(id, &[val, 0.0, 0.0, 0.0])
            }));
        }
