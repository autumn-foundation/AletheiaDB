...
mod sentry_tests {
    ...
    #[test]
    fn test_write_with_timeout_failure() {
        // This test verifies that write_with_timeout correctly returns an error
        // when the lock cannot be acquired within the timeout.
        // It covers the error path that integration tests might miss if not running with test-utils.
        ...
        // 2. Set timeout to very short duration (1ms)
        // Note: LOCK_TIMEOUT_MS is available as AtomicU64 in cfg(test)
        let old_timeout = LOCK_TIMEOUT_MS.swap(1, Ordering::Relaxed);

        // 3. Acquire a read lock to block the write lock
        let _read_guard = index.inner.read();

        // 4. Try to acquire write lock with timeout (should fail)
        let result = index.write_with_timeout();
        ...
        match result {
            Err(Error::Vector(VectorError::IndexError(msg))) => {
                assert!(
                    msg.contains("Timed out waiting for index write lock"),
                    "Unexpected error message: {}",
                    msg
                );
            }
            Ok(_) => panic!("Expected IndexError due to timeout, got Ok"),
            Err(e) => panic!("Expected IndexError due to timeout, got {:?}", e),
        }
    }

    #[test]
    fn test_add_rollback_timeout_unit() {
        ...
        // Wait for T1 to reach the ADD hook (approx)
        thread::sleep(Duration::from_millis(100));

        // Step 2: T2 runs and wins the ID
        TEST_ADD_DELAY_MS.store(0, Ordering::Relaxed); // Let T2 run fast
        ...
        // Reset flags
        TEST_ADD_DELAY_MS.store(0, Ordering::Relaxed);
