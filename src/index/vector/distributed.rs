...
/// (typically represented by a `MockVectorNodeClient`). If failures exceed the configured
...
///
/// The `NodeConnection` wraps a `MockVectorNodeClient` with a `NodeCircuitBreaker`
...
impl DistributedVectorIndex {
    ...
    pub fn new(config: DistributedVectorConfig, clients: Vec<Arc<MockVectorNodeClient>>) -> Result<Self> {
        config.validate()?;
...
///
/// `MockVectorNodeClient` is an in-memory client
...
impl MockVectorNodeClient {
    ...
    /// Find k most similar vectors on this remote node.
