...
/// (typically represented by a [`RemoteVectorNode`]). If failures exceed the configured
...
///
/// The `NodeConnection` wraps a [`RemoteVectorNode`] with a [`NodeCircuitBreaker`]
...
/// # fn main() {
/// use aletheiadb::index::vector::distributed::{
///     RemoteVectorNode, NodeConnection, CircuitBreakerConfig
/// };
/// use aletheiadb::index::vector::DistanceMetric;
/// use std::sync::Arc;
///
/// let client = Arc::new(RemoteVectorNode::new(0, 384, DistanceMetric::Cosine));
...
impl NodeConnection {
    ...
    pub fn new(client: Arc<RemoteVectorNode>, circuit_config: CircuitBreakerConfig) -> Self {
        Self {
            client,
    ...
    where
        F: FnOnce(&RemoteVectorNode) -> Result<T>,
    {
...
/// [`RemoteVectorNode`] for the required abstraction to interface with remote nodes.
///
/// # Examples
///
/// ```rust
...
/// use std::sync::Arc;
/// use aletheiadb::index::vector::distributed::{
///     DistributedVectorIndex, DistributedVectorConfig, VectorNodeConfig, RemoteVectorNode
/// };
/// use aletheiadb::index::vector::DistanceMetric;
///
...
///     .with_node(VectorNodeConfig::new(0, "node0:9000"));
///
/// let client = Arc::new(RemoteVectorNode::new(0, 128, DistanceMetric::Cosine));
...
impl DistributedVectorIndex {
    ...
    pub fn new(config: DistributedVectorConfig, clients: Vec<Arc<RemoteVectorNode>>) -> Result<Self> {
        config.validate()?;
...
///
/// `RemoteVectorNode` implements the [`RemoteVectorNode`] trait in-memory
...
/// # #[cfg(feature = "semantic-search")]
/// # fn main() -> aletheiadb::core::error::Result<()> {
/// use aletheiadb::index::vector::distributed::{RemoteVectorNode, RemoteVectorNode};
/// use aletheiadb::index::vector::DistanceMetric;
/// use aletheiadb::core::id::NodeId;
///
/// let client = RemoteVectorNode::new(0, 128, DistanceMetric::Cosine);
///
...
impl RemoteVectorNode {
    ...
    fn compute_similarity(&self, a: &[f32], b: &[f32]) -> f32 {
        match self.metric {
            ...
            other => panic!(
                "RemoteVectorNode does not support {:?} distance metric",
                other
            ),
        }
    }
...
mod tests {
    ...
    fn create_test_clients(num_nodes: usize) -> Vec<Arc<RemoteVectorNode>> {
        (0..num_nodes)
            .map(|i| {
                Arc::new(RemoteVectorNode::new(
                    i as u16,
                    4,
                    DistanceMetric::Cosine,
                ))
    ...
    fn test_create_distributed_index() -> Result<()> {
        ...
        let clients: Vec<Arc<RemoteVectorNode>> = clients.into_iter().collect();
    ...
    fn test_mock_client_add_search() -> Result<()> {
        let client = RemoteVectorNode::new(0, 4, DistanceMetric::Cosine);
    ...
    fn test_node_connection_execute_with_circuit_open() {
        let client = Arc::new(RemoteVectorNode::new(0, 4, DistanceMetric::Cosine));
