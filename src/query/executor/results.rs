...
///
/// Paths are typically generated by traversal queries (like shortest path) and
/// preserve the sequence of nodes and edges visited.
///
/// # Examples
///
/// ```rust
/// # use aletheiadb::core::NodeId;
/// # use aletheiadb::query::executor::{EntityId, Path};
/// let n1 = NodeId::new(1).unwrap();
/// let n2 = NodeId::new(2).unwrap();
/// let path: Path = vec![EntityId::Node(n1), EntityId::Node(n2)];
/// ```
pub type Path = Vec<EntityId>;

/// Entity identifier (node or edge).
///
/// This enum allows mixed sequences of entities to be represented,
/// such as in a traversal `Path` (internal).
...
///
/// Results can either contain full entity data ([`Node`], [`Edge`])
/// or just their identifiers ([`NodeId`], [`EdgeId`]) depending on the
/// query's projection requirements. This allows avoiding costly data fetches
...
///
/// This represents a single unit of output from a query execution pipeline.
/// It wraps a core [`EntityResult`] with optional metadata like vector similarity
/// scores, traversal paths, or temporal timestamps.
///
/// # Examples
///
/// ```rust
...
///
/// The query engine operates lazily. Query execution does not truly begin
/// until you start consuming this struct. You can stream results row-by-row
/// by implementing `IntoIterator` over its results, or collect everything
/// at once using [`QueryResults::collect_all`].
...
/// ```rust
/// # use aletheiadb::core::NodeId;
/// # use aletheiadb::query::executor::QueryResult;
/// let node_1 = NodeId::new(1).unwrap();
/// let node_2 = NodeId::new(2).unwrap();
///
/// // Create a query result with explicit IDs and similarity scores
/// let result = QueryResult::with_nodes(vec![node_1, node_2])
///     .with_scores(vec![0.9, 0.4]);
///
