...
impl QueryResults {
    ...
    /// # Performance
    ///
    /// ⚡ Bolt Optimization: This method processes the iterator in a single pass,
    /// avoiding the allocation of an intermediate vector of all rows.
    /// It initializes and pads property vectors on-the-fly only when necessary,
    /// significantly reducing memory consumption and improving cache locality for large result sets.
    pub fn collect_structured(mut self) -> Result<QueryResult> {
        ...
        while let Some(row) = self.iterator.next() {
            ...
            if score.is_some() && scores.is_none() {
                ...
                vec.resize(row_idx, 0.0);
                scores = Some(vec);
            }
            if let Some(ref mut s_vec) = scores {
                s_vec.push(score.unwrap_or(0.0));
            }
            ...
            if let Some(ref mut p_vec) = paths {
                p_vec.push(path.unwrap_or_default());
            }

            if timestamp.is_some() && versions.is_none() {
                let mut vec = Vec::with_capacity(lower);
                let pad_version = VersionId::new(0).expect("0 is valid");
                vec.resize(row_idx, pad_version);
                versions = Some(vec);
            }
            if let Some(ref mut v_vec) = versions {
