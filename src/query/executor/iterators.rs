...
/// }
/// ```
/// Iterator for sequential node scans at a specific point in time.
///
/// # Why?
/// Used when querying the entire graph (or a specific label) as it existed
/// historically. Similar to `NodeScanIterator` but reconstructs each node
/// state via `HistoricalStorage`.
///
/// # Memory Considerations
/// Like `NodeScanIterator`, it collects all node IDs upfront.
///
/// # Examples
///
/// ```rust
/// # use aletheiadb::query::executor::{TemporalNodeScanIterator, ResultIterator};
/// # use aletheiadb::storage::current::CurrentStorage;
/// # use aletheiadb::storage::historical::HistoricalStorage;
/// # use aletheiadb::core::temporal::time;
/// # use parking_lot::RwLock;
...
/// let now = time::now();
/// let node_ids = vec![];
/// let iter = aletheiadb::query::executor::TemporalNodeScanIterator::new(
///     node_ids,
///     now,
///     now,
///     historical,
...
/// ```
/// Iterator that performs a graph traversal from input nodes.
///
/// # Why?
/// Used to execute `MATCH (a)-[r]->(b)` graph queries. It takes an upstream iterator
/// that produces source nodes, and performs edge lookups to find connected nodes up
/// to a specified depth, handling both temporal and non-temporal queries.
///
/// # Examples
///
/// ```rust
...
/// # let current = Arc::new(CurrentStorage::new());
/// # let historical = Arc::new(RwLock::new(HistoricalStorage::new()));
/// // Traverses outward looking for "KNOWS" relationships, 1 hop deep.
/// let iter = aletheiadb::query::executor::TraversalIterator::new(
///     Box::new(EmptyIterator),
///     aletheiadb::query::ir::Direction::Outgoing,
///     Some("KNOWS".to_string()),
///     1, // depth
///     current,
///     historical,
///     None, // no temporal context
/// );
/// ```
...
/// Iterator that filters results based on a predicate.
///
/// # Why?
/// Used to apply a `Predicate` to the `QueryRow`s generated by an upstream iterator.
/// It evaluates the condition lazily, pulling from the input stream only as needed,
/// making it efficient for queries with `WHERE` clauses that eliminate many rows.
///
/// # Examples
///
/// ```rust
/// # use aletheiadb::query::executor::{FilterIterator, ResultIterator};
/// # use aletheiadb::query::planner::physical::PhysicalOp;
/// # use aletheiadb::query::executor::EmptyIterator;
/// # use aletheiadb::query::Predicate;
/// // Example of setting up a filter. In practice, this wraps a NodeScanIterator or similar.
/// let iter = FilterIterator::new(Box::new(EmptyIterator), Predicate::eq("name", "Alice"));
/// ```
...
/// Iterator that buffers and reranks results based on vector similarity.
///
/// # Why?
/// Used for hybrid search strategies where an initial filter (like a label match)
/// is applied, and the resulting nodes must then be sorted by semantic similarity
/// to a target vector.
///
/// This operator is **blocking/eager**: it must consume the entire input stream,
/// calculate similarities, sort them, and then yield the top `k` results, meaning
/// it breaks the purely lazy pipeline.
///
/// # Examples
///
/// ```rust
/// # use aletheiadb::query::executor::{VectorRerankIterator, ResultIterator};
/// # use aletheiadb::query::executor::EmptyIterator;
/// # use aletheiadb::storage::current::CurrentStorage;
/// # use std::sync::Arc;
/// # let current = Arc::new(CurrentStorage::new());
/// let target_embedding = vec![0.1, 0.2, 0.3];
/// let iter = VectorRerankIterator::new(
///     Box::new(EmptyIterator),
///     target_embedding.into(),
///     10, // top k
///     current,
///     None, // optional property key
/// );
/// ```
...
/// ```
/// Iterator that restricts the number of results returned, optionally skipping an initial offset.
///
/// # Why?
/// Used to implement `LIMIT` and `OFFSET` clauses. By intercepting the pull-based
/// stream and halting early, it prevents upstream operators from performing
/// unnecessary work, which is critical for query performance.
///
/// # Examples
///
/// ```rust
/// # use aletheiadb::query::executor::{LimitIterator, ResultIterator};
/// # use aletheiadb::query::executor::EmptyIterator;
/// // Skips the first 10 results, then yields a maximum of 5.
/// let iter = LimitIterator::new(Box::new(EmptyIterator), 10, 5);
/// ```
...
/// Iterator that conditionally strips internal provenance metadata.
///
/// # Why?
/// Some operations, like Temporal queries, attach metadata (like `valid_time`)
/// to rows. For users who only want the raw entities, this iterator acts as a
/// final pipeline stage to cleanly remove that metadata before returning the
/// result to the client.
///
/// # Examples
///
/// ```rust
/// # use aletheiadb::query::executor::{ProvenanceFilterIterator, ResultIterator};
/// # use aletheiadb::query::executor::EmptyIterator;
/// // Wrap an execution pipeline, keeping provenance metadata
/// let keep_metadata = true;
/// let iter = ProvenanceFilterIterator::new(Box::new(EmptyIterator), keep_metadata);
/// ```
...
/// Iterator that transforms results by keeping only specified properties.
///
/// # Why?
/// Used to implement `SELECT` style queries. Reduces memory overhead and serialization
/// cost by creating new nodes that contain only the subset of properties explicitly
/// requested by the query.
///
/// # Examples
///
/// ```rust
/// # use aletheiadb::query::executor::{ProjectIterator, ResultIterator};
/// # use aletheiadb::query::executor::EmptyIterator;
/// // Retain only the "name" and "age" properties
/// let iter = ProjectIterator::new(Box::new(EmptyIterator), vec!["name".to_string(), "age".to_string()]);
/// ```
...
/// Iterator that scans nodes sequentially, filtering by a specific property value.
///
/// # Why?
/// A fallback physical operator used when a query includes a `WHERE key = value` clause,
/// but no secondary index exists for that property. It performs a full scan of the
/// storage, making it slower but ensuring correct results.
///
/// # Examples
///
/// ```rust
...
/// # use std::sync::Arc;
/// # let current = Arc::new(CurrentStorage::new());
/// // Scans all "Person" nodes looking for an age of 30.
/// let iter = aletheiadb::query::executor::PropertyScanIterator::new(
///     "Person".to_string(), // Requires string now
///     "age".to_string(),
///     &aletheiadb::query::ir::PredicateValue::Int(30),
///     current
/// );
/// ```
