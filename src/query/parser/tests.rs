...
use crate::query::parser::EmbeddingRef;
use crate::query::parser::PredicateExpr;
...
fn test_parse_between() {
    let query = Parser::parse("BETWEEN '2024-01-01' AND '2024-12-31' MATCH (n) RETURN n").unwrap();
...
fn test_parse_where_and() {
    let query = Parser::parse("MATCH (n) WHERE n.age > 18 AND n.name = 'Alice' RETURN n").unwrap();
...
fn test_parse_where_grouped() {
    let query = Parser::parse("MATCH (n) WHERE (n.a = 1 OR n.b = 2) AND n.c = 3 RETURN n").unwrap();
...
fn test_parse_rank_by_similarity_without_top() {
    let query =
        Parser::parse("MATCH (a)-[:KNOWS]->(b) RANK BY SIMILARITY TO $embedding RETURN b").unwrap();
