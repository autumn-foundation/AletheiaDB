...
impl CurrentStorage {
    ...
    /// Returns an iterator to avoid allocating a Vec for large graphs,
    /// improving memory efficiency during persistence operations.
    pub(crate) fn all_edges(&self) -> impl Iterator<Item = impl std::ops::Deref<Target = Edge> + '_> + '_ {
    ...
    pub fn create_snapshot(
        ...
        let edges: Vec<Arc<Edge>> = self.indexes.iter_edges().map(|e| Arc::new(e.clone())).collect();
