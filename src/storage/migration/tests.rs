...
use crate::storage::migration::AtomicMigrationStats;
use std::sync::atomic::Ordering;
use std::time::{SystemTime, UNIX_EPOCH};

use crate::core::version::FastHashMap;
use crate::storage::migration::MigrationPolicyBuilder;
...
use crate::storage::migration::MigrationWithLsnResult;
...
fn test_migrate_node_versions() {
    ...
    let versions: Vec<NodeVersion> = (1..=10).map(|i| create_test_node_version(i, 100)).collect();
...
fn test_identify_candidates_respects_min_hot_versions() {
    ...
    let candidates = service.identify_node_candidates(&versions, &heads, &counts, Instant::now());
...
fn test_progress_tracking_callback() {
    ...
    let versions: Vec<NodeVersion> = (1..=12).map(|i| create_test_node_version(i, 100)).collect();
...
fn test_migration_run_stats_updated() {
    ...
    let versions: Vec<NodeVersion> = (1..=50).map(|i| create_test_node_version(i, 100)).collect();
...
fn test_migrate_edge_versions() {
    ...
    let versions: Vec<EdgeVersion> = (1..=10).map(|i| create_test_edge_version(i, 200)).collect();
...
fn test_identify_edge_candidates_respects_min_hot_versions() {
    ...
    let candidates = service.identify_edge_candidates(&versions, &heads, &counts, Instant::now());
