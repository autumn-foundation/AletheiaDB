...
fn test_observer_triggered_on_node_anchor_creation() {
    ...
    storage.add_observer(Arc::new(move |event: &StorageEvent| {
        match event {
...
fn test_observer_filtering() {
    ...
    let node_anchor_count = Arc::new(AtomicUsize::new(0));
    let nc = node_anchor_count.clone();
    ...
    assert_eq!(node_anchor_count.load(Ordering::SeqCst), 1);
}
...
fn test_observer_receives_correct_event_data() {
    ...
    assert_eq!(collected.len(), 2); // NodeAnchorCreated + NodeVersionCreated

    // Check anchor event
    let anchor_event = collected
        .iter()
...
fn test_observer_error_doesnt_block_storage() {
    ...
    storage.add_observer(Arc::new(|_event: &StorageEvent| {
