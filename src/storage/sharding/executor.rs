    ...
    fn serialize_traversal_plan(&self, plan: &TraversalPlan) -> Vec<u8> {
        ...
        if plan.steps.is_empty() {
            ...
            for shard_id in &plan.involved_shards {
                ...
                data.extend_from_slice(&(0u32).to_le_bytes()); // 0 labels
                data.push(0); // false
            }
            data
        } else {
...
mod tests {
    ...
    fn test_execute_traversal() {
        ...
        executor.register_client(shard0, client0.clone());
        executor.register_client(shard1, client1.clone());
        ...
        // The query data should not be empty (must contain at least the number of involved shards / steps).
