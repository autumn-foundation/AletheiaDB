...
/// Aggregated metric readout detailing a sharding strategy's effectiveness.
///
/// Contains precise distribution scores tracking the locality of nodes/edges
/// per-shard, expected runtime latencies caused by data scattering, and
/// the total disk overhead derived from required replications.
///
/// # Examples
/// ```rust
/// use aletheiadb::storage::sharding::simulation::SimulationResult;
///
/// let result = SimulationResult::new(1000, 5000, 3);
...
/// Analyzes structural disruption ("cuts") where an edge connects two
/// nodes residing on entirely separate storage instances.
///
/// Each cut fundamentally multiplies execution latency by enforcing
/// network transit protocols vs raw local SSD/Memory iteration.
...
/// Extrapolated performance bounds projecting execution durations of query hops.
///
/// Multiplies the structural edge-cut metrics by empirical network delays
/// to flag strategies prone to severe latency degradation.
#[derive(Debug, Clone, Default)]
...
/// Estimates total disk consumption imposed by edge replication rules across nodes.
///
/// Because edges spanning two shards are cloned on both the source and target
/// partition to facilitate immediate backwards navigation without RPC calls,
/// poor topological distribution artificially balloons total bytes stored.
#[derive(Debug, Clone, Default)]
...
/// Specifies volume and topological distributions governing the generated graph.
///
/// Modifies the mock graph generated by `ShardingSimulation` to closely resemble
/// a target data distribution scheme.
#[derive(Debug, Clone)]
...
/// Distribution weights representing explicit topological connections.
#[derive(Debug, Clone)]
...
/// Execution framework assessing distribution policies across multi-node deployments.
///
/// Models theoretical nodes and edges distributed via `ShardingStrategy` implementations.
/// Emits comprehensive latency and storage impact diagnostics, enabling administrators
/// to mathematically verify a domain-based mapping strategy prior to deploying live instances.
