...
mod tests {
    ...
    #[test]
    fn test_recover_pending_transactions() {
        let coordinator = ShardCoordinator::new(test_config());
        let shards = vec![ShardId::new(0).unwrap()];
        let tx_id = TxId::new(100);
        let commit_ts = crate::core::hlc::HybridTimestamp::new(1000, 0).unwrap();

        // 1. Manually inject a commit decision into the log
        {
            let mut log = coordinator.commit_log.write().unwrap();
        ...
        assert_eq!(result.recovered[0], tx_id);
    }
}
