...
pub(crate) fn persist_vector_indexes(
    ...
    for info in vector_indexes_info {
        ...
        // Get the index, config, and vector count
        // Note: We don't extract mappings here anymore. HnswIndex::save handles mappings internally
        // and atomically to ensure consistency.
        let (index, config, vector_count) = current
        ...
        // This saves both the index and its ID mappings atomically
...
pub(crate) fn load_vector_indexes(
    ...
    use crate::storage::index_persistence::vector::load_vector_meta;
    ...
    for entry in entries {
        ...
        let index = if usearch_path.exists() {
            // Load existing index
            // This now loads mappings internally from usearch.mappings (saved by HnswIndex::save)
        ...
        // Note: We no longer load mappings.idx or call restore_mapping.
        // HnswIndex::load is self-contained and restores mappings atomically
        // from the companion file generated by HnswIndex::save.
