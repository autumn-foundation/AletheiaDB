...
use crate::storage::index_persistence::{
    ...
    persist_vector_indexes, spawn_background_persistence_thread,
};
...
impl GallifreyDB {
    ...
    pub fn persist_indexes(&self) -> Result<()> {
        ...
        persist_string_interner(manager, self.persistence_tracker.as_deref())?;

        // 2. Save graph index
        persist_graph_index(&self.current, manager, self.persistence_tracker.as_deref())?;

        // 3. Save vector indexes
        persist_vector_indexes(&self.current, manager, self.persistence_tracker.as_deref())?;
        ...
        persist_temporal_index(
            ...
            &self.temporal_indexes,
            manager,
            self.persistence_tracker.as_deref(),
        )?;
