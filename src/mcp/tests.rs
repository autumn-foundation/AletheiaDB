...
mod node_tests {
    ...
    fn test_invalid_node_edge_id_parsing() {
        ...
        assert!(
            server
                .handle_update_node(invalid_node.clone())
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_delete_node(invalid_node.clone())
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_delete_node_cascade(invalid_node.clone())
                .is_error
                .unwrap_or(false)
        );
        ...
        assert!(
            server
                .handle_get_edge(invalid_edge.clone())
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_update_edge(invalid_edge.clone())
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_delete_edge(invalid_edge.clone())
                .is_error
                .unwrap_or(false)
        );
        ...
        assert!(
            server
                .handle_get_outgoing_edges(invalid_node.clone())
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_get_incoming_edges(invalid_node.clone())
                .is_error
                .unwrap_or(false)
        );
        assert!(
        ...
        assert!(
            server
                .handle_get_node_history(invalid_node.clone())
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_diff_node_versions(invalid_node.clone())
                .is_error
                .unwrap_or(false)
        );
        ...
        assert!(
            server
                .handle_get_edge_history(invalid_edge.clone())
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_diff_edge_versions(invalid_edge.clone())
                .is_error
                .unwrap_or(false)
        );
    }
    ...
    fn test_invalid_argument_parsing_exhaustive() {
        ...
        assert!(
            server
                .handle_create_node(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_update_node(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_delete_node(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_delete_node_cascade(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_list_nodes(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_count_nodes(json!({}))
                .is_error
                .unwrap_or(false)
        );
        ...
        assert!(
            server
                .handle_create_edge(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_update_edge(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_delete_edge(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_list_edges(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_count_edges(json!({}))
                .is_error
                .unwrap_or(false)
        );
        ...
        assert!(
            server
                .handle_get_outgoing_edges(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_get_incoming_edges(json!({}))
                .is_error
                .unwrap_or(false)
        );
        ...
        assert!(
            server
                .handle_find_similar(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_enable_vector_index(json!({}))
                .is_error
                .unwrap_or(false)
        );
        ...
        assert!(
            server
                .handle_get_node_at_time(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_get_edge_at_time(json!({}))
                .is_error
                .unwrap_or(false)
        );
        ...
        assert!(
            server
                .handle_get_node_at_valid_time(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_get_node_at_transaction_time(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_get_node_history(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_diff_node_versions(json!({}))
                .is_error
                .unwrap_or(false)
        );
        ...
        assert!(
            server
                .handle_get_edge_at_valid_time(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_get_edge_at_transaction_time(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_get_edge_history(json!({}))
                .is_error
                .unwrap_or(false)
        );
        assert!(
            server
                .handle_diff_edge_versions(json!({}))
                .is_error
                .unwrap_or(false)
        );
