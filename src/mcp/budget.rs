...
fn is_ranked_tool(tool: &str) -> bool {
    matches!(
        ...
        "find_similar"
            | "hybrid_query"
            // `semantic_search` reuses the exact `find_similar` k-NN path, so
            // its ranked `results` must enjoy the same protection: never
            // dropped or reordered to meet a token budget (Issue #2906).
            | "semantic_search"
