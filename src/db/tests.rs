...
mod query_tests {
    ...
    mod hybrid_tests {
        ...
        fn test_find_similar_as_of_basic() {
            ...
            let alice_embedding = [1.0f32, 0.0, 0.0, 0.0];
            let results = find_similar_as_of(&db, &alice_embedding, 10, timestamp)
                .expect("Query should succeed");
        ...
        fn test_find_similar_as_of_respects_k_limit() {
            ...
            let results = find_similar_as_of(&db, &query_embedding, 2, timestamp)
                .expect("Query should succeed");
        ...
        fn test_find_similar_as_of_temporal_consistency() {
            ...
            db.write(|tx: &mut WriteTransaction| {
                tx.update_node(
                    node,
                    PropertyMapBuilder::new()
                        .insert_vector("embedding", &[0.0f32, 1.0, 0.0, 0.0])
    ...
    #[cfg(test)]
    mod semantic_pathfinding_tests {
        use crate::api::transaction::WriteTransaction;
        ...
        fn test_semantic_pathfinding_time_travel() {
            ...
            let (_, t_edges) = db
                .write_with_timestamp(|tx: &mut WriteTransaction| {
            ...
            let (_, t_delete) = db
                .write_with_timestamp(|tx: &mut WriteTransaction| tx.delete_node_cascade(middle))
                .unwrap();
            let _t1 = t_delete;
